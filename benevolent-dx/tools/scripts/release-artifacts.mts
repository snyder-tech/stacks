...
const releaseTag =
  tagFlagIndex >= 0 ? process.argv[tagFlagIndex + 1] : process.env['RELEASE_TAG'];
...
function verify() {
  ...
  for (const packageDir of packageDirs) {
    ...
    if (!pkg.name.startsWith('@snyder-tech/bdx-analog-')) {
...
function smoke() {
  verify();
  pack();
  const tarballs = readdirSync(artifactsDir).filter((entry) => entry.endsWith('.tgz'));
